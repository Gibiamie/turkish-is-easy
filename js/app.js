...
import { esc, progressBar, visual, explanationSheet, pct } from './uiComponents.js';
...
const topicVisual={
  alphabet:{icon:'ABC',tone:'blue'},
  root_words:{icon:'-lik',tone:'green'},
  meaning_builder:{icon:'🧩',tone:'purple'},
  whose_builder:{icon:'★',tone:'sky'},
  plural_builder:{icon:'🦆',tone:'amber'},
  accusative_builder:{icon:'🎒',tone:'aqua'}
};
...
function ensureCelebrationStyles(){
  ...
  style.textContent=`
    ...
    .butterfly-burst span{position:absolute;font-size:22px;line-height:1;filter:drop-shadow(0 6px 8px rgba(15,23,42,.18));opacity:0;animation:butterflyFly 1.4s ease-out forwards;}
    .butterfly-burst span:nth-child(1){--x:-70px;--y:-96px;animation-delay:.02s;}
    .butterfly-burst span:nth-child(2){--x:-28px;--y:-118px;animation-delay:.10s;}
    .butterfly-burst span:nth-child(3){--x:28px;--y:-118px;animation-delay:.06s;}
    .butterfly-burst span:nth-child(4){--x:70px;--y:-96px;animation-delay:.14s;}
    @keyframes butterflyFly{0%{opacity:0;transform:translate(-50%,0) scale(.55) rotate(0deg);}18%{opacity:1;}70%{opacity:1;transform:translate(calc(-50% + var(--x)),var(--y)) scale(1.05) rotate(14deg);}100%{opacity:0;transform:translate(calc(-50% + var(--x)),calc(var(--y) - 26px)) scale(.7) rotate(-16deg);}}
    @media (prefers-reduced-motion: reduce){.butterfly-burst span{animation:none;opacity:0;}}
  `;
  document.head.appendChild(style);
}
function butterflyBurst(){ return `<div class="butterfly-burst" aria-hidden="true"><span>🦋</span><span>🦋</span><span>🦋</span><span>🦋</span></div>`; }
function bottomNav(active='dashboard'){
  return `<nav class="bottom-nav" aria-label="App navigation">
    <button class="${active==='dashboard'?'active':''}" data-action="home"><span>🏠</span><b>Dashboard</b></button>
...
function renderHome(){
  ...
  const topicCards=TOPICS.map(topic=>{
    ...
    const meta=topicVisual[topic.id]||{icon:'★',tone:'blue'};
  ...
  app.innerHTML=`<main class="home-page">
    ...
    <section class="module-grid">${topicCards}</section>${isQA?renderQA(state):''}${bottomNav('dashboard')}
  </main>`;
}
...
function renderLesson(){
  ...
  app.innerHTML=`<main class="lesson-page">${body}<nav class="lesson-nav"><button class="btn secondary" data-action="prev" ${state.index===0?'disabled':''}>${esc(t(lang,'previous'))}</button><button class="btn secondary" data-action="home">Dashboard</button>${endButton}</nav></main>`;
}
function lessonHeader(title,p,total){ return `<header class="lesson-head"><button class="close-x" data-action="home" aria-label="Dashboard">×</button><h1>${esc(title)}</h1><span class="streak-pill">🔥 7</span><div class="lesson-progress"><span style="width:${p}%"></span></div><b>${state.index+1} / ${total}</b></header>`; }
function layaGuide(text){ return `<section class="laya-guide"><div class="mini-laya">🐬</div><div><strong>Laya guide</strong><p>${esc(text)}</p></div></section>`; }
...
function renderSettings(){
  ...
  sheetRoot.innerHTML=`<div class="sheet-backdrop" data-action="close-sheet"><div class="sheet compact-sheet" role="dialog" aria-modal="true"><h2>${esc(t(lang,'settings'))}</h2><div class="settings-line"><span>Profile</span><strong>${esc(profileName)}</strong></div><div class="settings-line"><span>Learning path</span><strong>${esc(lockedLabel)}</strong></div><label>${esc(t(lang,'learnerMode'))}</label><div class="seg-row">${modes}</div><button class="btn secondary full" data-action="profile-screen">${esc(t(lang,'changeProfile'))}</button><button class="btn danger full" data-action="reset-progress">Reset progress</button><button class="btn primary full" data-action="close-sheet">${esc(t(lang,'close'))}</button></div></div>`;
}
function play(path){
...
function reset(){ if(confirm(state.language==='id'?'Hapus progres profil ini?':'Delete this profile progress?')){ state.progress.clear(); sheetRoot.innerHTML=''; render(); } }
